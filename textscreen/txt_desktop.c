...
#define MAX_GARBAGE 128
static txt_window_t *garbage_pile[MAX_GARBAGE];
static int garbage_count = 0;

void TXT_PileGarbage(txt_window_t *garbage)
{
	if (garbage_count + 1 == MAX_GARBAGE)
		return;

	garbage_pile[garbage_count++] = garbage;
}

static void TXT_ThrowGarbage()
{
	int i;
	for (i=0; i<garbage_count; i++)
		OLD__TXT_CloseWindow(garbage_pile[i]);
