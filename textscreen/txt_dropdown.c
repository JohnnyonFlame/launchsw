...
{
	int table_h = TXT_GETTABLEH(list->num_values);
    int result;

    if (ValidSelection(list))
    {
        result = list->widget.y - 1;
    }
    else
    {
        result = list->widget.y - 1 - (table_h / 2);
    }
    ...
    else if (result + table_h > (TXT_SCREEN_H - 3))
    {
        result = TXT_SCREEN_H - table_h - 3;
    }
...
static void ItemSelected(TXT_UNCAST_ARG(button), TXT_UNCAST_ARG(callback_data))
{
    ...
    *callback_data->list->variable = callback_data->table->selected_y - 1;
...
static void OpenSelectorWindow(txt_dropdown_list_t *list)
{
    ...
    pane   = TXT_NewScrollPane(0, TXT_GETTABLEH(list->num_values), table);
    ...
    for (i=0; i<list->num_values; ++i)
    {
        ...
        TXT_SignalConnect(pane, "closed", FreeCallbackData, data);
